export const createContext = async (flashcardId: string, text: string) => {
  const res = await fetch("/api/contexts", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ flashcard_id: flashcardId, text }),
  });
...
export const updateContext = async (id: string, text: string) => {
